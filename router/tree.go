...
type node struct {
	...
	indices   string
	children  []*node
	handle    httpcontext.HandlerFunc
	priority  uint32
}
...
func (n *node) addRoute(path string, handle httpcontext.HandlerFunc) {
	fullPath := path
	n.priority++
...
func (n *node) insertChild(numParams uint8, path, fullPath string, handle httpcontext.HandlerFunc) {
		...
		if len(path) > len(n.path) {
			if path[:len(n.path)] == n.path {
				...
				case param:
					...
					if p == nil {
						// lazy allocation
						p = make(httpcontext.Params, 0, n.maxParams)
					}
					i := len(p)
